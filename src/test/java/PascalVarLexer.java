...
public class PascalVarLexer {
    ...
    static class PascalVarLexerException extends Exception {
        PascalVarLexerException(String message) {
            super(message);
        }
    }
    ...
    private TokenPattern[] tokenPatterns = {
        new TokenPattern(TokenType.VAR, "^var"), 
        new TokenPattern(TokenType.COLON, "^[:]"), 
        new TokenPattern(TokenType.SEMICOLON, "^[;]"), 
        new TokenPattern(TokenType.COMMA, "^[,]"), 
        new TokenPattern(TokenType.NAME, "^[a-zA-Z][a-zA-Z0-9_]*")
    };
    ...
    public Token nextToken() throws PascalVarLexerException {
        ...
        throw new PascalVarLexerException ("Can't parse token at: ..." + input.substring(0, Math.min(10, input.length())) + (input.length() > 10 ? "..." : ""));
    }
}
